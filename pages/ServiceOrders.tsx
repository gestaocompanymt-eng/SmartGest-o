...
import { 
  Plus, FileText, ChevronDown, ChevronUp, X, DollarSign, Edit2, Share2, Wrench, MapPin, Camera, Trash2, Image as ImageIcon, CheckCircle2, Save, Layers, Settings, Building2, RefreshCcw, Play
} from 'lucide-react';
                    ...
                    <div className="space-y-2">
                      ...
                      {os.photos_before?.length > 0 && (
                        <div className="flex gap-2 overflow-x-auto py-2">
                          {os.photos_before.map((p, i) => <img key={i} src={p} className="h-16 w-16 object-cover rounded-lg border shadow-sm" />)}
                        </div>
                      )}
                    </div>
                    <div className="space-y-2">
                       ...
                       {os.photos_after?.length > 0 && (
                        <div className="flex gap-2 overflow-x-auto py-2">
                          {os.photos_after.map((p, i) => <img key={i} src={p} className="h-16 w-16 object-cover rounded-lg border border-slate-700" />)}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                   ...
                   <button type="button" onClick={() => setAssignmentType('equipment')} className={`py-3 rounded-xl text-[9px] font-black uppercase border transition-all ${assignmentType === 'equipment' ? 'bg-blue-600 text-white border-blue-600 shadow-lg shadow-blue-500/20' : 'bg-white text-slate-400 border-slate-200'}`}>Equipamento</button>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Fotos da Ocorrência (Antes)</label>
              ...
              {(editingOS || selectedCondoId) && (
                <div className="pt-6 border-t space-y-4 animate-in zoom-in-95">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-blue-600 uppercase tracking-widest ml-1">Ações de Reparo / Vistoria Realizadas</label>
                    <textarea name="actions" defaultValue={editingOS?.actions_performed} rows={4} className="w-full px-5 py-4 bg-slate-900 text-blue-100 border border-slate-800 rounded-2xl text-[10px] font-mono outline-none shadow-inner" placeholder="Relate detalhadamente o que foi executado..."></textarea>
                  </div>

                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-blue-600 uppercase tracking-widest ml-1">Fotos da Execução / Finalização (Depois)</label>
                    <div className="flex gap-2 flex-wrap">
                      {photosAfter.map((p, i) => (
                        <div key={i} className="relative group">
                      ...
                      <label className="w-24 h-24 border-2 border-dashed border-blue-200 bg-blue-50/50 rounded-xl flex flex-col items-center justify-center text-blue-400 cursor-pointer hover:border-blue-500 hover:text-blue-600 transition-all">
                        <Camera size={24} />
                        <span className="text-[8px] font-black mt-1 uppercase">Tirar Foto</span>
