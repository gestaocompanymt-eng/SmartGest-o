...
const Dashboard: React.FC<{ data: AppData; updateData: (d: AppData) => void }> = ({ data, updateData }) => {
  ...
  const filteredAppointments = useMemo(() => {
    ...
    // Para técnicos, talvez queiram ver apenas as deles ou todas? Geralmente admin vê todas.
    // Vamos mostrar todas ordenadas por data
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-10">
      ...
      {/* 1. Atividades Recentes - Movido para a parte superior conforme solicitado */}
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        ...
        <div className="divide-y divide-slate-100">
          ...
          {filteredOSList.length === 0 && (
            <div className="p-8 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">Nenhuma atividade recente.</div>
          )}
        </div>
      </div>

      {/* 2. Agenda de Visitas - Nova seção */}
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
              ...
              <div key={appt.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 flex flex-col justify-between hover:border-blue-300 transition-colors group">
                <div className="space-y-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center text-[10px] text-slate-500 font-bold uppercase tracking-tight">
                      <UserIcon size={12} className="mr-1 text-slate-400" /> {tech?.name || 'Não atribuído'}
                    </div>
                  </div>
                  <p className="text-[10px] text-slate-600 font-medium bg-white p-2 rounded-lg border border-slate-100 italic">
                    "{appt.description}"
                  </p>
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-300 flex flex-col">
            ...
            <form onSubmit={handleAppointmentSubmit} className="p-6 space-y-5">
              ...
              <div className="space-y-1">
                ...
                <textarea required name="description" defaultValue={editingAppointment?.description} rows={2} placeholder="Ex: Manutenção preventiva das bombas..." className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium"></textarea>
              </div>
              <div className="space-y-1">
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button type="submit" className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-slate-900/20 active:scale-95">
                  {editingAppointment ? 'Atualizar Visita' : 'Confirmar Agendamento'}
                </button>
              </div>
            </form>
          </div>
