    ...
    <div className="space-y-6 md:space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-xl shadow-blue-900/20 relative overflow-hidden">
             <div className="absolute top-0 right-0 p-4 opacity-10">
                <Activity size={80} />
             </div>
             <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                   <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-400">Live Telemetria</h4>
                ...
                {telemetrySummary.latest ? (
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                       <div>
                          <p className="text-4xl font-black">{telemetrySummary.latest.percentual}%</p>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tight mt-1">Última Leitura Geral</p>
                       </div>
                       <div className={`p-4 rounded-3xl ${telemetrySummary.criticalCount > 0 ? 'bg-red-500' : 'bg-blue-600 shadow-lg shadow-blue-600/30'}`}>
                          <Droplets size={28} />
                       </div>
                    </div>
                    ...
                    {telemetrySummary.criticalCount > 0 && (
                      <div className="bg-red-500/20 border border-red-500/50 p-3 rounded-xl flex items-center space-x-2">
                        ...
                        <span className="text-[9px] font-black uppercase text-red-100">Existem {telemetrySummary.criticalCount} pontos em nível baixo!</span>
                      </div>
                    )}
                    ...
                    <button onClick={() => navigate('/reservatorios')} className="w-full py-4 bg-white/10 hover:bg-white/20 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center group">
                       Monitorar Todos <ChevronRight size={14} className="ml-2 group-hover:translate-x-1 transition-transform" />
                    </button>
                  </div>
                ) : (
                  <div className="py-12 text-center space-y-4">
                     <WifiOff size={40} className="mx-auto text-slate-600" />
                     <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Aguardando ESP32...</p>
                  </div>
                )}
             </div>
          </div>
