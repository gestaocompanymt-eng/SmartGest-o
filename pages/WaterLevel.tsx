  ...
  return (
    <div className="space-y-8 pb-12">
        ...
        <div className="flex space-x-2 w-full md:w-auto">
          {isAdminOrTech && orphanedSensors.length > 0 && (
            <button 
              ...
              className={`flex-1 md:flex-none px-4 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center space-x-2 ${showDiscovery ? 'bg-amber-500 text-white shadow-lg' : 'bg-white border text-amber-600 border-amber-200'}`}
            >
              <Search size={16} />
              <span>Detectados ({orphanedSensors.length})</span>
            </button>
          )}
          <button 
      ...
      {showDiscovery && isAdminOrTech && (
        <div className="bg-amber-50 border-2 border-amber-200 rounded-[2.5rem] p-8 animate-in zoom-in-95 duration-200">
           <div className="flex items-center space-x-4 mb-6">
              <div className="p-4 bg-amber-500 text-white rounded-[1.5rem] shadow-xl shadow-amber-500/20"><Wifi size={24} /></div>
              <div>
                 <h3 className="text-lg font-black text-amber-900 uppercase leading-none">Placas Detectadas</h3>
                 <p className="text-[10px] font-bold text-amber-700 uppercase mt-1">Copie o ID e vincule a um sistema para começar o monitoramento.</p>
              </div>
           </div>
           
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {orphanedSensors.map(sensor => (
                <div key={sensor.id} className="bg-white p-6 rounded-[2rem] border border-amber-200 shadow-sm relative overflow-hidden group hover:border-amber-400 transition-all">
                   <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Cpu size={60} /></div>
                   <span className="text-[8px] font-black text-amber-500 uppercase tracking-widest bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100">ID Serial Captado</span>
                   <p className="text-xl font-black text-slate-900 select-all font-mono mt-2 mb-4">{sensor.condominio_id}</p>
                   <div className="flex justify-between items-center pt-4 border-t border-slate-50">
                      <div className="flex flex-col">
                         <span className="text-[8px] font-black text-slate-400 uppercase">Visto pela última vez</span>
          ...
          <div key={entry.condo.id} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {entry.points.map((point: MonitoringPoint) => {
                ...
                const isOffline = latest ? (new Date().getTime() - new Date(latest.created_at).getTime() > 300000) : true; // Offline se sem sinal por 5 min
                      ...
                      {latest ? (
                        ...
                        <div className="py-24 flex flex-col items-center justify-center text-center space-y-5 bg-slate-50 rounded-[3rem] border-4 border-dashed border-slate-200">
                          ...
                          <div className="px-10">
                             ...
                             <p className="text-sm font-black text-amber-600 font-mono mt-2 bg-amber-50 px-3 py-1 rounded-lg border border-amber-100 inline-block">{point.device_id}</p>
                          </div>
                        </div>
                      )}
                    </div>
